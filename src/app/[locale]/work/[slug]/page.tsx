...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: tProjects(`${slug}.summary`),
  };
}
...
export default async function CaseStudyPage({ params }: Props) {
  ...
  const meta = [
    { label: t('clientLabel'),   value: project.client },
    { label: t('categoryLabel'), value: tProjects(`${project.slug}.category`) },
    { label: t('yearLabel'),     value: String(project.year) },
    { label: t('tagsLabel'),     value: (tProjects.raw(`${project.slug}.tags`) as string[]).join(', ') },
  ];
      ...
      <header style={{
        ...
        <div style={{ position: 'relative', maxWidth: 'var(--grid-max)', width: '100%', marginInline: 'auto' }}>
          <span style={{
            ...
            {tProjects(`${project.slug}.category`)} — {project.year}
          </span>
          <h1 style={{
